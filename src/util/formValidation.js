...
const registerValidation = (username, age, email, password, password2) => {
  ...
  // email => valid email
  // password => 1 uppercase, 1 lowercase, min 6, max 16, 1 special char, no white space, no username
  ...
  if (password === '' || password2 === '') {
    ...
    if (password.length > 16) {
      errors.push('Password length must be less than 16 characters long.');
    }
  ...
  if (!validator.isEmail(email)) {
    errors.push('Please use a valid email adress.');
  }
